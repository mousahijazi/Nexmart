            ...
            <div className="relative overflow-hidden rounded-2xl bg-[var(--color-green-dark)] dark:bg-[#0f2e25] text-white p-7 border border-transparent dark:border-[#22332e] shadow-sm">
                <div className="relative z-10 p-6">
                    ...
                    <h1 className="text-2xl sm:text-3xl font-extrabold">
                        {tDash("welcome", { name: ordersLoading ? "" : firstName})}
                    </h1>
            ...
            {ordersLoading ? (
                <div className="bg-white dark:bg-[#18221f] border border-[var(--color-border)] dark:border-[#22332e] rounded-2xl p-6">
                    <div className="flex items-center justify-between mb-5">
                        <div className="w-32">
                            <div className="h-7 w-24 rounded bg-gray-200 dark:bg-zinc-700/60 animate-pulse" />
                            <div className="h-4 w-32 rounded bg-gray-200 dark:bg-zinc-800 animate-pulse mt-1" />
                        </div>

                        <div className="w-9 h-9 rounded-xl bg-[#EEF4F1] dark:bg-[#122A23] flex items-center justify-center">
                            <div className="w-5 h-5 rounded bg-gray-200 dark:bg-zinc-700/60 animate-pulse" />
                        </div>
                    </div>

                    <div className="flex flex-col">
                        {[1, 2, 3].map((item) => (
                            <div key={item} className="flex items-center justify-between gap-4 py-4 border-b last:border-b-0 border-[var(--color-divider)] dark:border-[#22332e]">
                                <div>
                                    <div className="h-5 w-28 rounded bg-gray-200 dark:bg-zinc-700/60 animate-pulse" />
                                    <div className="h-4 w-20 rounded bg-gray-100 dark:bg-zinc-800 animate-pulse mt-1" />
                                </div>

                                <div className="h-4 w-14 rounded-full bg-gray-200 dark:bg-zinc-700/60 animate-pulse" />
                            </div>
                        ))}
                    </div>
                </div>
